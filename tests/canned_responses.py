...
_OUTLINE_MARKER = "extract scientific papers into concise structured notes"
_OUTLINE_RESPONSE = '{"problem":"Current workflows are slow.","method":"The paper proposes a compact method.","finding":"It improves efficiency.","limitation":"It lacks broader validation."}'
_IDEA_MARKER = "turns scientific papers into practical ideas"
_IDEA_RESPONSE = '["Try the method on your current benchmark.","Adapt the paper into a workflow automation step.","Design a small ablation around the core claim."]'
_DAILY_SUMMARY_MARKER = "synthesize a set of papers into a short set of feasible, high-value ideas"
_DAILY_SUMMARY_RESPONSE = '[{"idea":"Test the strongest method on your current benchmark this week.","innovation":"It combines a compact method with your current workflow instead of reproducing the paper as-is.","feasibility":"The papers already show an efficiency gain and the setup matches your existing benchmark process.","evidence":["Sample Paper Title: the paper proposes a compact method.","Sample Paper Title: it reports an efficiency improvement."],"first_step":"Run a small benchmark slice and compare against your current baseline."},{"idea":"Automate one repeated workflow step using the paper ideas.","innovation":"It turns a paper-level technique into a reusable workflow primitive.","feasibility":"The implementation scope is narrow and can be validated on one recurring task first.","evidence":["Sample Paper Title: the method is compact enough to integrate quickly."],"first_step":"Choose one repeated task and build a minimal automation wrapper around the core method."},{"idea":"Run a targeted ablation on the main claimed improvement.","innovation":"It tests whether the claimed gain survives under your own task assumptions.","feasibility":"Ablations can be limited to one benchmark and one metric.","evidence":["Sample Paper Title: the main finding is an efficiency improvement."],"first_step":"Define one metric, one baseline, and one reduced variant to compare this week."}]'
...
def _stub_chat_create(**kwargs):
    ...
    if _OUTLINE_MARKER in request_str:
        return _make_chat_response(_OUTLINE_RESPONSE)
    if _IDEA_MARKER in request_str:
        return _make_chat_response(_IDEA_RESPONSE)
    if _DAILY_SUMMARY_MARKER in request_str:
        return _make_chat_response(_DAILY_SUMMARY_RESPONSE)
